...
func installFlags(flags *pflag.FlagSet, c *Opts) {
	flags.StringVar(&c.KubeConfigsPath, "kubeconfigs", c.KubeConfigsPath, "a directory containing kubeconfig files for connecting to the Kubernetes API server")
	...
	flags.StringVar(&c.KubeTLSCertPath, "tls-cert-path", c.KubeTLSCertPath, "kubernetes client cert path")
	flags.StringVar(&c.KubeTLSKeyPath, "tls-key-path", c.KubeTLSKeyPath, "kubernetes client key path")
	flags.StringVar(&c.KubeApiServer, "kube-api-server", c.KubeApiServer, "kubernetes api server")
	flags.IntVar(&c.TotalNodeCnt, "total-node-cnt", c.TotalNodeCnt, "total node count")
