...
func NewCommand(ctx context.Context, s *provider.Store, c Opts) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "stress",
		Short: "Simulate many kubelet instances talking to the cloud.",
		Long: `Runs many instances of a node controller using the mock provider to provide a realistic simulation of
		load on the kubernetes master.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runStressCommand(ctx, s, c)
		},
	}
...
func runStressCommand(ctx context.Context, s *provider.Store, c Opts) error {
...
func getKubeconfigs(dir string) (map[string]string, error) {
	...
	for _, file := range files {
		...
		nodeName := file.Name()[:len(file.Name())-len(".kubeconfig")]
		kubeconfigs[nodeName] = path.Join(dir, file.Name())
	}
...
func runNode(ctx context.Context, nodeName string, client	*kubernetes.Clientset, s *provider.Store, c Opts) error {
	...
	go func() {
		for err := nodeRunner.Run(ctx); err != nil; err = nodeRunner.Run(ctx) {
			log.G(ctx).Error(err)
		}
	}()
